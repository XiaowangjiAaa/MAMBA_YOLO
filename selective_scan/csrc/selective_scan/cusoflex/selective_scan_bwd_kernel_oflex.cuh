...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    auto& smem_load_dout = reinterpret_cast<typename Ktraits::BlockLoadOutputT::TempStorage&>(smem_);
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        output_t dout_vals_load[kNItems];
        __syncthreads();
        load_input<Ktraits>(u, u_vals, smem_load, params.seqlen - chunk * kChunkSize);
        __syncthreads();
        load_input<Ktraits>(delta, delta_vals_load, smem_load, params.seqlen - chunk * kChunkSize);
        __syncthreads();
        load_input<Ktraits>(dout, dout_vals_load, smem_load_dout, params.seqlen - chunk * kChunkSize);
        u -= kChunkSize;
...
inline void selective_scan_bwd_launch_impl(SSMParamsBwd &params, cudaStream_t stream) {
...
void selective_scan_bwd_launch(SSMParamsBwd &params, cudaStream_t stream) {
    ...
    if (is_even_len) {
        if (delta_softplus) {
            selective_scan_bwd_launch_impl<kNThreads, kNItems, true, true, input_t, weight_t, output_t>(params, stream);
        } else {
            selective_scan_bwd_launch_impl<kNThreads, kNItems, true, false, input_t, weight_t, output_t>(params, stream);
        }
    } else {
        if (delta_softplus) {
            selective_scan_bwd_launch_impl<kNThreads, kNItems, false, true, input_t, weight_t, output_t>(params, stream);
        } else {
            selective_scan_bwd_launch_impl<kNThreads, kNItems, false, false, input_t, weight_t, output_t>(params, stream);
        }
    }
}
