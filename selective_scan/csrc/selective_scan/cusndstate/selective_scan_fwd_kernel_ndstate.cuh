...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    for (int chunk = 0; chunk < params.n_chunks; ++chunk) {
        ...
        input_t *out_cur = out + chunk * kChunkSize;
        __syncthreads();
...
// 帮助函数：用具体类型的函数消除 MSVC 对内联 lambda 的 C2975 编译歧义
template<int kNThreads, int kNItems, bool kIsEvenLen, bool kDeltaSoftplus, typename input_t, typename weight_t>
inline void selective_scan_fwd_launch_impl(SSMParamsBase &params, cudaStream_t stream) {
    using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, kIsEvenLen, kDeltaSoftplus, input_t, weight_t>;
    const int kSmemSize = Ktraits::kSmemSize + params.dstate * sizeof(typename Ktraits::scan_t);
...
void selective_scan_fwd_launch(SSMParamsBase &params, cudaStream_t stream) {
    ...
    if (is_even_len) {
        if (delta_softplus) {
            selective_scan_fwd_launch_impl<kNThreads, kNItems, true, true, input_t, weight_t>(params, stream);
        } else {
            selective_scan_fwd_launch_impl<kNThreads, kNItems, true, false, input_t, weight_t>(params, stream);
        }
    } else {
        if (delta_softplus) {
            selective_scan_fwd_launch_impl<kNThreads, kNItems, false, true, input_t, weight_t>(params, stream);
        } else {
            selective_scan_fwd_launch_impl<kNThreads, kNItems, false, false, input_t, weight_t>(params, stream);
        }
    }
}
