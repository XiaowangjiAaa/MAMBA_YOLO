...
void selective_scan_fwd_launch(SSMParamsBase &params, cudaStream_t stream) {
    ...
    // 用显式 switch 分支直接传入 bool 字面量，彻底避开 MSVC Lambda 捕获及 C2975/C2131 模板报错
    switch (is_even_len ? 1 : 0) {
        case 1: {
            switch (delta_softplus ? 1 : 0) {
                case 1: {
                    using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, true, true, input_t, weight_t>;
                ...
                case 0: {
                    using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, true, false, input_t, weight_t>;
                    constexpr int kSmemSize = Ktraits::kSmemSize + Ktraits::MaxDState * sizeof(typename Ktraits::scan_t);
                    dim3 grid(params.batch, params.dim);
                    auto kernel = &selective_scan_fwd_kernel<Ktraits>;
                    if (kSmemSize >= 48 * 1024) {
        ...
        case 0: {
            switch (delta_softplus ? 1 : 0) {
                case 1: {
                    using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, false, true, input_t, weight_t>;
                    constexpr int kSmemSize = Ktraits::kSmemSize + Ktraits::MaxDState * sizeof(typename Ktraits::scan_t);
                    dim3 grid(params.batch, params.dim);
                    auto kernel = &selective_scan_fwd_kernel<Ktraits>;
                    if (kSmemSize >= 48 * 1024) {
                ...
                case 0: {
                    using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, false, false, input_t, weight_t>;
                    constexpr int kSmemSize = Ktraits::kSmemSize + Ktraits::MaxDState * sizeof(typename Ktraits::scan_t);
                    dim3 grid(params.batch, params.dim);
                    auto kernel = &selective_scan_fwd_kernel<Ktraits>;
                    if (kSmemSize >= 48 * 1024) {
